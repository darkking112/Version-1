...
function getDays() {
    ...
    const updatedDays = days.map(day => day.toISOString().split('T')[0]);
    return updatedDays;
}
...
async function checkEligibility(id) {
    ...
    const query = `SELECT Appointment_Date FROM appointment WHERE Appointment_Date > ? AND Appointment_Status = "Approved" AND Donor_ID = ?;`
...
const viewAvailablAppointments = async function (req, res) {
    const { DCID, donorID } = req.body;
    let temp = await checkEligibility(donorID);
    if(temp) {
        let values = getDays();
        values.push(DCID);
        ...
        return res.json("You are unable to book and appointment due to a confirmed donation within the previous 90 days")
