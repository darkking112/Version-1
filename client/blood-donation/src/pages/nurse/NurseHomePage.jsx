...
function NurseHomePage() {
    ...
    useEffect(() => {
        fetch('http://localhost:5000/nurse/appointment-list', {
            method: 'POST',
            headers: {
    ...
    const handleApprovement = (appointment) => {
        fetch(`http://localhost:5000/nurse/change-appointment-status/${appointment.Appointment_ID}`, {
            method: 'POST'
        })
            .then(response => response.json())
            .then(data => {
    ...
    const confrimAppointment = (type, amount) => {
        fetch(`http://localhost:5000/nurse/approve-appointment/${approvedAppointment.Appointment_ID}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    const rejectAppointment = (appointment) => {
        fetch(`http://localhost:5000/nurse/reject-appointment/${appointment.Appointment_ID}`, {
            method: 'POST'
        })
            .then(response => response.json())
            .then(data => {
