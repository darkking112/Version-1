...
function DCHomePage() {
    ...
    useEffect(() => {
        fetch(`http://localhost:5000/donationcenteradmin/nurse-list/${DC.DC_Admin_ID}`)
    ...
    const handleConfirmRemove = () => {
        ...
        fetch(`http://localhost:5000/donationcenteradmin/remove-nurse/${currentNurse.User_ID}`, {
            method: 'DELETE'
        })
        .then(response => {
            if (response.ok) {
    ...
    const handleAddNurse = (newNurse) => {
        console.log(DC);
        fetch(`http://localhost:5000/donationcenteradmin/add-nurse/${DC.DC_Admin_ID}`, {
            method: 'POST',
            headers: {
