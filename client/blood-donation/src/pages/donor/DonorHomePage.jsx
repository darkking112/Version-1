...
function DonorHomePage() {
    ...
    const fetchBookedAppointment = async () => {
        try {
            const response = await fetch('http://localhost:5000/donor/view-booked-appointment', {
                method: 'POST',
    ...
    const handleCancelAppointment = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/donor/cancel-appointment/${bookedAppointment[1][0].Appointment_ID}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' }
            });
